...
import { sendChatMessage_Schema, uploadFiles, generateVectorDb as apiGenerateVectorDb } from "@/lib/api-service"
import { Textarea } from "@/components/ui/textarea"
import { Lightbulb, ChevronDown, ChevronUp } from "lucide-react"

// Update the GlobalAgent component to handle file transitions
export default function GlobalAgent() {
  ...
  useEffect(() => {
    ...
    if (currentConversation && currentConversation.agentType === "global" && currentConversation.messages.length > 0) {
      setShowChat(true)
  ...
  // Add this function to the GlobalAgent component to handle the animation when transitioning to chat
  // Add this right after the useEffect hooks
  ...
  const transitionToChat = (conversationId: string) => {
    ...
    if (files.length > 0 && vectorDbGenerated && dbButtonRef.current) {
      ...
      floatingIcon.className = "fixed z-50 bg-cyan-500 rounded-full p-2 shadow-lg shadow-cyan-500/30"
  ...
  // Default schema suggestion for global agent
  const defaultSchema = `
    ## Company_name
    company name
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!conversationId || !conversations[conversationId] || conversations[conversationId].agentType !== "global") {
      conversationId = addConversation("global")
    ...
    const schemaToSend = schema ? schema : "No schema provided, look for the information required"
    try {
  ...
  const generateVectorDb = async () => {
      ...
      if (!convId || conversations[convId]?.agentType !== "global") {
        convId = addConversation("global")
      }
      ...
      try {
        ...
        const uploadResp = await uploadFiles(files, convId, "global")
        ...
        const dbResp = await apiGenerateVectorDb(filePaths, convId, "global")
  ...
  const handleUpload = async () => {
    ...
    if (
      !conversationId ||
      conversations[conversationId]?.agentType !== "global"
    ) {
      conversationId = addConversation("global")
    }
    ...
    try {
      ...
      const resp = await uploadFiles(files, conversationId, "global")
  ...
  const currentConversation = getCurrentConversation()
  const messages = currentConversation?.agentType === "global" ? currentConversation.messages : []
  const conversationFiles = currentConversation?.agentType === "global" ? currentConversation.files : undefined
      ...
      {!showChat ? (
          ...
          <Card className="border-0 bg-gray-800/30 backdrop-blur-md shadow-xl overflow-hidden border-t border-gray-700/50">
            <CardContent className="pt-6">
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2, duration: 0.5 }}>
                <h2 className="text-2xl font-bold mb-2 flex items-center gap-2 text-cyan-300">
                  <Sparkles className="h-5 w-5" />
                  Global Agent
                </h2>
                <p className="mb-6 text-gray-400">
                  Ask questions, upload files, and generate a vector database for enhanced responses.
                </p>
              </motion.div>
                ...
                {files.length > 0 && (
                  ...
                  >
                    <Button
                      ...
                      //className="w-full …"
                      className="w-full bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white border-0 shadow-lg shadow-cyan-500/20 transition-all duration-300"
                      //disabled={agentState === "generating vector database" || vectorDbGenerated}
                    >
                      {agentState === "generating vector database" ? (
                        <div className="flex items-center justify-center w-full">
                          <div className="relative w-full">
                            ...
                            <div className="mt-1.5 w-full bg-cyan-900/30 rounded-full h-1.5 overflow-hidden">
                              <motion.div
                ...
                >
                  <div className="flex gap-2">
                    <Input
                      ...
                      className="flex-1 bg-gray-700/50 border-gray-600 text-white focus:border-cyan-400 focus:ring-cyan-400/20 transition-all duration-300"
                    />
                    <Button
                      type="submit"
                      className="bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white border-0 shadow-lg shadow-cyan-500/20 transition-all duration-300"
                    >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    >
                      ...
                      <span>Response Schema {showSchemaInput ? "(Hide)" : "(Show)"}</span>
                    </Button>
                    ...
                    {showSchemaInput && (
                      <Button
                        ...
                        className="text-cyan-400 border-cyan-500/30 hover:bg-cyan-500/10 transition-colors duration-300 flex items-center gap-1"
                      >
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between">
                            <p className="text-xs text-gray-400">
                              Specify a schema to structure the response (optional)
                            </p>
                          </div>
                          <Textarea
                            ref={schemaInputRef}
                            placeholder="Enter JSON schema for response structure..."
                            value={schema}
                            onChange={(e) => setSchema(e.target.value)}
                            className="min-h-[150px] font-mono text-sm bg-gray-800/70 border-gray-600 text-white focus:border-cyan-400 focus:ring-cyan-400/20 transition-all duration-300"
                          />
                        </div>
                      </motion.div>
                    )}
        ...
        >
          {vectorDbGenerated && dbIconPosition.x !== 0 && !showChat && (
            <motion.div
              className="fixed z-50 bg-cyan-500 rounded-full p-2 shadow-lg shadow-cyan-500/30"
              initial={{
                x: dbIconPosition.x,
                y: dbIconPosition.y,
                scale: 1,
          ...
          <ChatInterface
            ...
            onSendMessage={async (newMessage) => {  // Usamos función asíncrona
              ...
              if (
                ...
                // Si tenemos archivos, agrégalos a la conversación
                if (files.length > 0 && vectorDbGenerated) {
                  const fileInfo = {
                    count: files.length,
                    names: files.map((f) => f.name),
              ...
              try {
                const responseData = await sendChatMessage_Schema(
                  newMessage,
                  conversationId,
                  "global",
                  schema ? schema : "No schema provided, look for the information required"
                );
                if (responseData.status === "success") {
                  ...
                  addMessage(conversationId, {
                    role: "assistant",
                    content: `Error: ${responseData.response || "unknown"}`,
                  });
                }
              } catch (err) {
